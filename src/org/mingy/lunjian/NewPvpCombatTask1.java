...
public class NewPvpCombatTask1 extends TimerTask {
	...
	public NewPvpCombatTask1(CommandLine cmdline) {
						...
						if (postures != null) {
							for (Posture posture : postures) {
								...
								if (m.find()) {
									...
									String defender = null;
									if (posture.source > 0) {
										attacker = m.group(posture.source);
									}
									if (posture.target > 0) {
										defender = m.group(posture.target);
									}
									...
									if (!is_combo) {
										System.out.println("[VS] "
												...
												+ (defender != null ? " "
														+ defender : ""));
										if (vs_npc1 == null) {
											if (isNpc(attacker, vs1)) {
												vs_npc1 = attacker;
											} else if (isNpc(defender, vs1)) {
												vs_npc1 = defender;
											}
										}
										if (vs_npc2 == null) {
											if (isNpc(attacker, vs2)) {
												vs_npc2 = attacker;
											} else if (isNpc(defender, vs2)) {
												vs_npc2 = defender;
											}
										} else if (!isNpc(defender, vs2)) {
											vs_npc2 = null;
