...
public class AutoGuildTrigger implements Trigger {

	private static final Pattern PATTERN1 = Pattern
			.compile("^帮派使者：给我在.+内(战胜|杀|寻找)(.+)。任务所在地方好像是：(.+)你已经连续完成了\\d+个任务。你今天已完成(\\d+)/(\\d+)个任务。$");
	...
	private static final Pattern PATTERN3 = Pattern
			.compile("^恭喜你完成帮派任务，这是你连续完成的第\\d+个帮派任务！");
	...
	public boolean match(CommandLine cmdline, String message, String type,
		...
		if ("今天做的帮派任务已过量，明天再来。".equals(message)) {
			cmdline.closeTrigger("guild");
			return true;
		}
		...
		if (!m.find()) {
			...
			if (!m.find()) {
				...
				if (m.find()) {
					cmdline.sendCmd("clan scene;clan task");
					return true;
				} else {
					return false;
				}
		...
		if (times + 1 == limit) {
			cmdline.sendCmd("vip finish_clan");
		}
	...
	protected void process(CommandLine cmdline, String action, String target,
		...
		if ("华山村".equals(map) && "黑狗".equals(npc)) {
			...
			spec = true;
		} else if ("大昭寺".equals(map) && "护寺藏尼".equals(npc)) {
			spec = true;
		} else if ("桃花岛".equals(map) && "桃花岛弟子".equals(npc)) {
			spec = true;
		} else if ("大理".equals(map)
				&& ("采笋人".equals(npc) || "农夫".equals(npc) || "台夷商贩".equals(npc))) {
			spec = true;
		}
		...
		AutoGuildTask task = new AutoGuildTask(cmdline, action, rooms, npc,
				item, 0, 500, null);
	...
	private class AutoGuildTask extends TimerTaskDelegate {
		...
		public AutoGuildTask(CommandLine cmdline, String action,
			...
			if (state == 0) {
				if (rooms.isEmpty()) {
					...
					Runnable callback = new Runnable() {
						@Override
						public void run() {
							AutoGuildTask task = new AutoGuildTask(cmdline,
				...
				if ("战胜".equals(action)) {
					...
					if (target != null) {
						...
						cmdline.sendCmd("clan scene;clan submit_task");
						cmdline.stopTask(this);
					} else {
						state = 0;
					}
